...
import MatchThumbnails from '../MatchThumbnails'
...
const initialState = {
  ...
  selectedTabId: 'FRUIT',
  selectedTabImgs: imagesList.filter(eachItem => eachItem.category === 'FRUIT'),
  isResultsShow: false,
}
...
class MatchGame extends Component {
  ...
  onActiveTab = id => {
    this.setState({
      selectedTabId: id,
      selectedTabImgs: imagesList.filter(eachItem => eachItem.category === id),
    })
  }

  onThumbnailFunc = id => {
    const {matchId} = this.state
    const matchRandomIndex = Math.floor(Math.random() * imagesList.length)

    if (matchId === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        matchImg: imagesList[matchRandomIndex].imageUrl,
        matchId: imagesList[matchRandomIndex].id,
      }))
    } else {
  ...
  componentDidMount = () => {
    this.setTimeIntervalId = setInterval(this.timeStartsInSec, 1000)
  }
      ...
      <div className="bg-container">
        <nav className="nav-header">
          ...
          <div className="score-timer">
            <div className="score-timer score-container">
              <p className="score-text">Score: </p>
              <p className="score"> {score}</p>
            </div>
            <div className="score-timer timer-container">
              <img
                src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
                alt="timer"
                className="timer-img"
              />
              <p className="score timer-seconds">{secTime} sec</p>
            </div>
          </div>
        </nav>
        <div className="main-bg-container">
          {!isResultsShow && (
            <>
              <img src={matchImg} alt="match" className="main-game-img" />
              <ul className="tabs-ul-container">
                {tabsList.map(eachItem => (
                  <MatchGameTabs
                    onActiveTab={this.onActiveTab}
                    selectedTab={selectedTabId === eachItem.tabId}
                    key={eachItem.tabId}
                    eachItem={eachItem}
                  />
                ))}
              </ul>
              <ul className="tabs-ul-container thumbnail-ul-container">
                {selectedTabImgs.map(eachItem => (
                  <MatchThumbnails
                    onThumbnailFunc={this.onThumbnailFunc}
                    key={eachItem.id}
                    eachItem={eachItem}
                  />
                ))}
          ...
          {isResultsShow && (
            <ShowResults score={score} onReplayFunc={this.onReplayFunc} />
          )}
        </div>
      </div>
    )
